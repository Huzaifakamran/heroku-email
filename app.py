...
cur = conn.cursor()
cur.execute("SELECT user_identifier, user_message, bot_message, created_at from public.customer_dialogue_tb where date(created_at)=CURRENT_DATE")
...
#Monthly
monthly = conn.cursor()
monthly.execute("SELECT user_identifier, user_message, bot_message, created_at FROM public.customer_dialogue_tb WHERE created_at >= date_trunc('month', CURRENT_DATE) AND created_at < date_trunc('month', CURRENT_DATE) + INTERVAL '1 month';")
                            ...
                            <tr>
                                <td class="bee-performance-item" style="border: 0; font-weight: normal; background-color: #ffffff; mso-line-height-rule: exactly; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%;" valign="top">
                                    <table align="left" border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse; mso-table-lspace: 0pt; mso-table-rspace: 0pt; -ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%;">
                                       """.format(yesterday_date,current_date,current_date,dailyCount,weeklyCount,monthlyCount)
